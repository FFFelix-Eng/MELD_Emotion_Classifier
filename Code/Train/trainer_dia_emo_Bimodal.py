...
from util.metrics_old import compute_metrics, save_metrics, init_metrics_file
...
class Trainer_dia_emo:
    ...
    def compute_metrics_using_util(self, dataloader):
        ...
        with torch.no_grad():
            for idx, content in enumerate(dataloader):
                input_ids, aud, attention_mask, labels = content['text'].to(self.device), content['aud'].to(
                    self.device), content['attn_mask'].to(self.device), content['label'].to(self.device)
                sentence_mask = self.compute_sentence_mask(attention_mask)
                predictions = self.model(input_ids, aud, attention_mask)
        ...
        for epoch in range(epochs):
            ...
            for idx, content in enumerate(train_loader):
                input_ids, aud, attention_mask, labels = content['text'].to(self.device), content['aud'].to(
                    self.device), content['attn_mask'].to(self.device), content['label'].to(self.device)
                sentence_mask = self.compute_sentence_mask(attention_mask)
                outputs = self.model(input_ids, aud, attention_mask)
