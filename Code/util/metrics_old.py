...
def compute_metrics(predictions, labels, mask, num_classes=7):
    ...
    for preds_batch, labels_batch, masks_batch in zip(predictions, labels, mask):
        preds_batch = preds_batch.cpu() if isinstance(preds_batch, Tensor) else preds_batch
        labels_batch = labels_batch.cpu() if isinstance(labels_batch, Tensor) else labels_batch
        masks_batch = masks_batch.cpu() if isinstance(masks_batch, Tensor) else masks_batch

        if isinstance(masks_batch, list) and isinstance(masks_batch[0], Tensor):
